            ...
            <form action="{{ route('anggota.update', ['nomor_anggota' => $anggota->nomor_anggota]) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white shadow-md sm:rounded-lg p-6">
                        ...
                        <!-- Jenis Kelamin -->
                        <div class="mb-4" x-data="{ open: false, selected: '{{ old('jenis_kelamin', $anggota->jenis_kelamin) }}', options: ['Laki-laki', 'Perempuan'] }">
                        ...
                        <!-- Agama -->
                        <div class="mb-4" x-data="{ open: false, selected: '{{ old('agama', $anggota->agama) }}', options: ['Islam', 'Kristen', 'Katolik', 'Hindu', 'Buddha', 'Konghucu'] }">
                        ...
                        @php
                            $existingCombined = old('golongan_pramuka', $anggota->golongan_pramuka);
                            $parts = explode(' - ', $existingCombined, 2);
                            $existingGolongan = $parts[0] ?? '';
                            $existingTingkat  = $parts[1] ?? '';
                        @endphp
                        <div class="mb-4" x-data="{
                            openGol: false,
                            openTingkat: false,
                            golongan: '{{ $existingGolongan }}',
                            tingkat: '{{ $existingTingkat }}',
                            golonganOptions: {
                    ...
                    <div class="bg-white shadow-md sm:rounded-lg p-6 flex flex-col">
                        ...
                        <h2 class="text-base font-bold text-gray-700 mb-4 pb-2 border-b border-gray-200">Data Lainnya</h2>

                        <!-- Golongan Darah -->
                        <div class="mb-4" x-data="{ open: false, selected: '{{ old('golongan_darah', $anggota->golongan_darah) }}', options: ['A', 'B', 'AB', 'O'] }">
